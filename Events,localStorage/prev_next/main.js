...
//Встановлення початкового значення сторінки та ліміту (кількість об'єктів на сторінці).
let page = 1;
const limit = 10;

//Оголошення функції-обробника, яка отримує параметри page та limit.
const handler = (page, limit) => {
    ...
    wrapper.replaceChildren(...state); //бере всі елементи з state і замінює всі поточні дочірні елементи wrapper на нові елементи з масиву state. Це дозволяє оновлювати вміст wrapper згідно нового стану, який ми формуємо в state.
}

//Виклик функції при завантаженні сторінки для відображення початкового стану.
handler(page, limit);

//Додавання обробників подій для кнопок, які викликають функцію handler з новими параметрами при кожному кліку.
